...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonMotor : vCharacter, vIAnimatorStateInfoController
    {
        ...
        public virtual float staminaRecovery
        {
            get { return _staminaRecovery; }
            set { _staminaRecovery = value; }
        }
        ...
        public virtual float sprintStamina
        {
            get { return _sprintStamina; }
            set { _sprintStamina = value; }
        }

        [SerializeField] protected float _jumpStamina = 30f;

        public virtual float jumpStamina
        {
            get { return _jumpStamina; }
            set { _jumpStamina = value; }
        }
        ...
        [vEditorToolbar("Stamina", order = 2)] [SerializeField]
        protected float _maxStamina = 200f;
        ...
        public virtual float rollStamina
        {
            get { return _rollStamina; }
            set { _rollStamina = value; }
        }

        [vEditorToolbar("Events", order = 7)] public UnityEvent OnExitGround;
        ...
        [vEditorToolbar("Crouch", order = 3)] [Range(1, 2.5f)]
        public float crouchHeightReduction = 2f;

        [Range(1, 2f)] public float crouchColliderRadius = 1.5f;
        ...
        [Tooltip(
            "Use this to move character only in forward direction when in free locomotion, the character will rotate to input direction normaly but the movement will be based on character forward")]
        ...
        [vHelpBox(
            "FreeLocomotion: Rotate on any direction regardless of the camera \nStrafeLocomotion: Move always facing forward (extra directional animations)")]
        [SerializeField, FormerlySerializedAs("locomotionType")]
        protected LocomotionType _locomotionType = LocomotionType.FreeWithStrafe;

        public virtual LocomotionType locomotionType
        {
            get { return _locomotionType; }
            set { _locomotionType = value; }
        }

        public vMovementSpeed freeSpeed, strafeSpeed;

        [vSeparator("Extra Animation Settings")] [Tooltip("Use it for debug purposes")]
        ...
        [vHelpBox(
            "When 'Use RootMotion' is checked, make sure to reset all speeds to zero to use the original root motion velocity.")]
        ...
        [Tooltip("Smooth value for the Lean Movement animation")] [Range(0.01f, 0.1f)]
        public float leanSmooth = 0.05f;

        [Tooltip(
            "Check this to use the TurnOnSpot animations while the character is stading still and rotating in place")]
        ...
        [SerializeField]
        protected float _jumpHeight = 4f;
        ...
        [vSeparator("Falling")] [Tooltip("Speed that the character will move while airborne")] [SerializeField]
        protected float _airSpeed = 5f;

        public virtual float airSpeed
        {
            get { return _airSpeed; }
            set { _airSpeed = value; }
        }

        [Tooltip("Smoothness of the direction while airborne")] [SerializeField]
        protected float _airSmooth = 6f;

        public virtual float airSmooth
        {
            get { return _airSmooth; }
            set { _airSmooth = value; }
        }

        [Tooltip("Apply extra gravity when the character is not grounded")] [SerializeField]
        protected float _extraGravity = -10f;

        public virtual float extraGravity
        {
            get { return _extraGravity; }
            set { _extraGravity = value; }
        }

        [Tooltip("Limit of the vertical velocity when Falling")] [SerializeField]
        protected float _limitFallVelocity = -15f;

        public virtual float limitFallVelocity
        {
            get { return _limitFallVelocity; }
            set { _limitFallVelocity = value; }
        }
        ...
        [SerializeField]
        protected float _ragdollVelocity = -15f;

        public virtual float ragdollVelocity
        {
            get { return _ragdollVelocity; }
            set { _ragdollVelocity = value; }
        }

        [vSeparator("Fall Damage")] [SerializeField]
        protected float _fallMinHeight = 6f;

        public virtual float fallMinHeight
        {
            get { return _fallMinHeight; }
            set { _fallMinHeight = value; }
        }

        [SerializeField] protected float _fallMinVerticalVelocity = -10f;

        public virtual float fallMinVerticalVelocity
        {
            get { return _fallMinVerticalVelocity; }
            set { _fallMinVerticalVelocity = value; }
        }

        [SerializeField] protected float _fallDamage = 10f;

        public virtual float fallDamage
        {
            get { return _fallDamage; }
            set { _fallDamage = value; }
        }

        [vEditorToolbar("Roll", order = 4)] public bool useRollRootMotion = true;

        [Tooltip("Animation Transition from current animation to Roll")]
        public float rollTransition = .25f;

        [Range(1, 2.5f)] public float rollHeightReduction = 1.6f;
        [Range(1, 2f)] public float rollColliderRadius = 1.5f;

        [Tooltip("Can control the Roll Direction")]
        public bool rollControl = true;

        [Tooltip("Speed of the Roll Movement")] [SerializeField]
        protected float _rollSpeed = 0f;
        ...
        [Tooltip("Speed of the Roll Rotation")] [SerializeField]
        protected float _rollRotationSpeed = 20f;

        public virtual float rollRotationSpeed
        {
            get { return _rollRotationSpeed; }
            set { _rollRotationSpeed = value; }
        }

        [vHideInInspector("Roll use gravity influence")] [SerializeField]
        protected bool _rollUseGravity = true;

        public virtual bool rollUseGravity
        {
            get { return _rollUseGravity; }
            set { _rollUseGravity = value; }
        }

        [vHideInInspector("rollUseGravity")]
        [Tooltip("Normalized Time of the roll animation to enable gravity influence")]
        [SerializeField]
        protected float _rollUseGravityTime = 0.2f;

        public virtual float rollUseGravityTime
        {
            get { return _rollUseGravityTime; }
            set { _rollUseGravityTime = value; }
        }

        [Tooltip("Use the normalized time of the animation to know when you can roll again")]
        [Range(0, 1)]
        [SerializeField]
        protected float _timeToRollAgain = 0.75f;

        public virtual float timeToRollAgain
        {
            get { return _timeToRollAgain; }
            set { _timeToRollAgain = value; }
        }

        [Tooltip("Ignore all damage while is rolling, include Damage that ignore defense")] [SerializeField]
        protected bool _noDamageWhileRolling = true;

        public virtual bool noDamageWhileRolling
        {
            get { return _noDamageWhileRolling; }
            set { _noDamageWhileRolling = value; }
        }

        [Tooltip("Ignore damage that needs to activate ragdoll")] [SerializeField]
        protected bool _noActiveRagdollWhileRolling = true;

        public virtual bool noActiveRagdollWhileRolling
        {
            get { return _noActiveRagdollWhileRolling; }
            set { _noActiveRagdollWhileRolling = value; }
        }
        ...
        [vEditorToolbar("Grounded", order = 3)] [vSeparator("Ground")] [SerializeField] [vReadOnly()]
        ...
        [Tooltip(
            "Snaps the capsule collider to the ground surface, recommend when using complex terrains or inclined ramps")]
        public bool useSnapGround = true;

        [Range(0, 1)] public float snapPower = 0.5f;

        [Tooltip("Distance to became not grounded")] [Range(0, 10)]
        public float groundMinDistance = 0.1f;

        [Range(0, 10)] public float groundMaxDistance = 0.5f;

        [Tooltip("Max angle to walk")] [vSeparator("StopMove")]
        ...
        [vSeparator("Slope Limit")] [SerializeField]
        protected bool _useSlopeLimit = true;

        public virtual bool useSlopeLimit
        {
            get { return _useSlopeLimit; }
            set { _useSlopeLimit = value; }
        }

        [Range(30, 80)] [SerializeField] protected float _slopeLimit = 75f;

        public virtual float slopeLimit
        {
        ...
        public virtual float stopSlopeMargin
        {
            get { return _stopSlopeMargin; }
            set { _stopSlopeMargin = value; }
        }

        [SerializeField] protected float _SlopeSidewaysSmooth = 2f;

        public virtual float slopeSidewaysSmooth
        {
            get { return _SlopeSidewaysSmooth; }
            set { _SlopeSidewaysSmooth = value; }
        }

        [SerializeField] protected float _slopeMinDistance = 0f;

        public virtual float slopeMinDistance
        {
            get { return _slopeMinDistance; }
            set { _slopeMinDistance = value; }
        }

        [SerializeField] protected float _slopeMaxDistance = 1.5f;

        public virtual float slopeMaxDistance
        {
            get { return _slopeMaxDistance; }
            set { _slopeMaxDistance = value; }
        }

        [SerializeField] protected float _slopeLimitHeight = 0.2f;

        public virtual float slopeLimitHeight
        {
            get { return _slopeLimitHeight; }
            set { _slopeLimitHeight = value; }
        }


        protected float _slopeSidewaysSmooth;
        [HideInInspector] public bool steepSlopeAhead;

        [vSeparator("Slide On Slopes")] [SerializeField]
        protected bool _useSlide = true;

        public virtual bool useSlide
        {
            get { return _useSlide; }
            set { _useSlide = value; }
        }

        [Tooltip("Velocity to slide down when on a slope limit ramp")] [Range(0, 30)] [SerializeField]
        protected float _slideDownVelocity = 10f;

        public virtual float slideDownVelocity
        {
            get { return _slideDownVelocity; }
            set { _slideDownVelocity = value; }
        }

        [Tooltip("Smooth to slide down the controller")] [SerializeField]
        protected float _slideDownSmooth = 2f;

        public virtual float slideDownSmooth
        {
            get { return _slideDownSmooth; }
            set { _slideDownSmooth = value; }
        }

        [Tooltip("Velocity to slide sideways when on a slope limit ramp")] [Range(0, 1)] [SerializeField]
        protected float _slideSidewaysVelocity = 0.5f;

        public virtual float slideSidewaysVelocity
        {
            get { return _slideSidewaysVelocity; }
            set { _slideSidewaysVelocity = value; }
        }

        [Range(0f, 1f)] [Tooltip("Delay to start sliding once the character is standing on a slope")] [SerializeField]
        protected float _SlidingEnterTime = 0.2f;

        public virtual float slidingEnterTime
        {
            get { return _SlidingEnterTime; }
            set { _SlidingEnterTime = value; }
        }

        internal float _slidingEnterTime;

        [Range(0f, 1f)] [Tooltip("Delay to rotate once the character started sliding")] [SerializeField]
        protected float _RotateSlopeEnterTime = 0.1f;

        public virtual float rotateSlopeEnterTime
        {
            get { return _RotateSlopeEnterTime; }
            set { _RotateSlopeEnterTime = value; }
        }

        [Tooltip("Smooth to rotate the controller")] [SerializeField]
        protected float _rotateDownSlopeSmooth = 8f;

        public virtual float rotateDownSlopeSmooth
        {
            get { return _rotateDownSlopeSmooth; }
            set { _rotateDownSlopeSmooth = value; }
        }

        internal float _rotateSlopeEnterTime;

        [vSeparator("Step Offset")] [SerializeField]
        protected bool _useStepOffset = true;

        public virtual bool useStepOffset
        {
            get { return _useStepOffset; }
            set { _useStepOffset = value; }
        }
        ...
        [Range(0, 1)]
        [SerializeField]
        protected float _stepOffsetMaxHeight = 0.5f;

        public virtual float stepOffsetMaxHeight
        {
            get { return _stepOffsetMaxHeight; }
            set { _stepOffsetMaxHeight = value; }
        }

        [Tooltip(
            "Offset min height to walk on steps. Make sure to keep slight above the floor - YELLOW Raycast in front of the legs")]
        [Range(0, 1)]
        [SerializeField]
        protected float _stepOffsetMinHeight = 0f;

        public virtual float stepOffsetMinHeight
        {
            get { return _stepOffsetMinHeight; }
            set { _stepOffsetMinHeight = value; }
        }

        [Tooltip("Offset distance to walk on steps - YELLOW Raycast in front of the legs")]
        [Range(0, 1)]
        [SerializeField]
        protected float _stepOffsetDistance = 0.1f;

        public virtual float stepOffsetDistance
        {
            get { return _stepOffsetDistance; }
            set { _stepOffsetDistance = value; }
        }

        internal float _stopMoveWeight;

        internal virtual float stopMoveWeight
        {
            get { return _stopMoveWeight; }
            set { _stopMoveWeight = value; }
        }

        internal float _sprintWeight;

        internal virtual float sprintWeight
        {
            get { return _sprintWeight; }
            set { _sprintWeight = value; }
        }

        internal float groundDistance;
        public RaycastHit groundHit;

        [vEditorToolbar("Debug", order = 9)] [Header("--- Debug Info ---")]
        ...
        {
            get => _animatorStateInfos;
            protected set => _animatorStateInfos = value;
        }
        ...
        {
            get { return sprintOnlyFree && isSprinting ? false : _isStrafing; }
        ...
        public virtual float
            inputMagnitude { get; set; } // sets the inputMagnitude to update the animations in the animator controller
        ...
        /// set the current moveSpeed for the<seealso cref="MoveCharacter(Vector3)"/> method        
        ...
        public virtual float
            colliderHeight { get; set; } // storage capsule collider extra information                       
            ...
            lockSetMoveSpeed
        {
            get;
            set;
        } // locks the method to update the moveset based on the locomotion type, so you can modify externally

        protected bool _isStrafing { get; set; } // internally used to set the strafe movement

        public virtual bool
            lockInStrafe { get; set; } // locks the controller to only used the strafe locomotion type        

        public virtual bool forceRootMotion { get; set; } // force the controller to use root motion

        public virtual bool
            keepDirection { get; set; } // keeps the character direction even if the camera direction changes
            ...
            lockAnimMovement
        {
            get;
            set;
        } // internally used with the vAnimatorTag("LockMovement"), use on the animator to lock the movement of a specific animation clip        
            ...
            lockAnimRotation
        {
            get;
            set;
        } // internally used with the vAnimatorTag("LockRotation"), use on the animator to lock a rotation of a specific animation clip

        public virtual Vector3
            lastCharacterAngle { get; set; } //Last angle of the character used to calculate rotationMagnitude;
        ...
        public virtual Vector3
            colliderCenter { get; set; } // storage the center of the capsule collider info                
        ...
        /// </summary>
        public virtual float colliderRadiusDefault { get; protected set; }

        /// <summary>
        /// Default Height of the Character Capsule
        /// </summary>
        public virtual float colliderHeightDefault { get; protected set; }

        /// <summary>
        /// Default Center of the Character Capsule
        /// </summary>
        public virtual Vector3 colliderCenterDefault { get; protected set; }
        ...
        {
            get { return !blockApplyFallDamage && jumpMultiplier <= 1 && !customAction; }
        }
        ...
        public void TakeDamage(vDamage damage, bool isFire = false)
        {
            if (ishiting == false)
            {
                if (!isFire)
        ...
        public virtual void SetControllerMoveSpeed(vMovementSpeed speed)
        {
            ...
            {
                moveSpeed = Mathf.Lerp(moveSpeed, isSprinting ? speed.runningSpeed : speed.walkSpeed,
                    speed.movementSmooth * Time.fixedDeltaTime);
            }
            else
            {
                moveSpeed = Mathf.Lerp(moveSpeed, isSprinting ? speed.sprintSpeed : speed.runningSpeed,
                    speed.movementSmooth * Time.fixedDeltaTime);
            }
        }
        ...
        public virtual void MoveCharacter(Vector3 direction)
        {
            ...
            inputSmooth = Vector3.Lerp(inputSmooth, input,
                ...
                (useRootMotion ? vTime.deltaTime : vTime.fixedDeltaTime));

            if (lockMovement || isSliding || ragdolled || !isGrounded || isJumping || _rigidbody.isKinematic)
            {
                return;
            }
            ...
            Vector3 targetPosition = (useRootMotion ? animator.rootPosition : _rigidbody.position) + _direction *
                (moveSpeed * speedMultiplier) * (useRootMotion ? vTime.deltaTime : vTime.fixedDeltaTime);
        ...
        protected virtual void CheckStopMove(ref Vector3 targetVelocity)
        {
            ...
            bool checkStopMoveCondition =
                isGrounded && !isJumping && !isInAirborne && !applyingStepOffset && !customAction;
        ...
        protected virtual bool CheckStopMove(Vector3 direction, out RaycastHit hit)
        {
            ...
            switch (stopMoveCheckMethod)
            {
                ...
                case StopMoveCheckMethod.CapsuleCast:
                    ...
                    return Physics.CapsuleCast(p1, p2, _capsuleCollider.radius, direction, out hit, distance,
                        stopMoveLayer);
                default:
        ...
        protected virtual void SnapToGround(ref Vector3 targetVelocity, ref bool useVerticalVelocity)
        {
            ...
            var snapConditions = isGrounded && groundHit.collider != null && GroundAngle() <= slopeLimit &&
                                 !disableCheckGround && !isSliding && !isJumping && !customAction &&
                                 input.magnitude > 0.1f && !isInAirborne;
        ...
        protected virtual void CalculateStepOffset(Vector3 moveDir, ref Vector3 targetVelocity,
            ref bool useVerticalVelocity)
        {
            if (useStepOffset && isGrounded && !disableCheckGround && !isSliding && !isJumping && !customAction &&
                !isInAirborne)
            {
                ...
                if (Physics.SphereCast(ray, _capsuleCollider.radius * 0.5f, out stepOffsetHit,
                        (stepOffsetMaxHeight - stepOffsetMinHeight), stepOffsetLayer) &&
                    stepOffsetHit.point.y > transform.position.y)
                {
        ...
        {
            get => ((inputSmooth.sqrMagnitude + input.sqrMagnitude) > 0.1f ||
                    (input - inputSmooth).sqrMagnitude > 0.1f);
        }
        ...
        public virtual bool CanExitCrouch()
        {
            if (isCrouching)
            {
                ...
                if (Physics.SphereCast(ray2, radius, out groundHit, crouchHeadDetect - (colliderRadius * 0.1f),
                        autoCrouchLayer))
                {
                    return false;
                }
        ...
        {
            get { return isRolling && animatorStateInfos.GetCurrentNormalizedTime(0) >= timeToRollAgain; }
        }
        ...
        protected virtual void RollBehavior()
        {
            ...
            Vector3 deltaPosition = useRollRootMotion
                ? new Vector3(animator.deltaPosition.x, 0f, animator.deltaPosition.z)
        ...
        protected virtual void CheckGround()
        {
            ...
            if (groundDistance <= groundMinDistance || applyingStepOffset)
            {
                ...
                {
                    _rigidbody.AddForce(transform.up * (extraGravity * 2 * Time.fixedDeltaTime),
            ...
            else
            {
                if (groundDistance >= groundMaxDistance)
                {
                    ...
                    {
                        _rigidbody.AddForce(transform.up * extraGravity * Time.fixedDeltaTime,
                            ForceMode.VelocityChange);
                    }
                }
        ...
        {
            if (isGrounded || verticalVelocity > fallMinVerticalVelocity || !_canApplyFallDamage ||
                fallMinHeight == 0 || fallDamage == 0)
            {
                return;
            }
        ...
        protected virtual bool CheckForSlope(ref Vector3 targetVelocity)
        {
            if (debugWindow)
            {
                Debug.DrawLine(transform.position + Vector3.up * (_capsuleCollider.height * slopeLimitHeight),
                    transform.position + moveDirection.normalized *
                    (steepSlopeAhead
                        ? _capsuleCollider.radius + slopeMaxDistance
                        : _capsuleCollider.radius + slopeMinDistance), Color.red, 0.01f);
            }
            ...
            // DYNAMIC LINE
            if (Physics.Linecast(transform.position + Vector3.up * (_capsuleCollider.height * slopeLimitHeight),
                    transform.position + moveDirection.normalized *
                    (steepSlopeAhead
                        ? _capsuleCollider.radius + slopeMaxDistance
                        : _capsuleCollider.radius + slopeMinDistance), out slopeHitInfo, groundLayer))
            {
                ...
                if (hitAngle > slopeLimit && hitAngle < 85f)
                {
                    ...
                    var dir = Quaternion.AngleAxis(normalAngle.y > 0f ? 90f : -90, Vector3.up) * normal.normalized *
                              targetVelocity.magnitude;

                    if (Mathf.Abs(normalAngle.y) > stopSlopeMargin)
                    {
                        _slopeSidewaysSmooth = Mathf.Clamp(_slopeSidewaysSmooth - Time.deltaTime * slopeSidewaysSmooth,
                            0f, 1f);
                    }
                    else
                    {
        ...
        protected virtual void SlideOnSteepSlope()
        {
            if (useSlide && isGrounded && GroundAngle() > slopeLimit && !disableCheckGround)
            {
                if (_slidingEnterTime <= 0f || isSliding)
                {
                    ...
                    if (!Physics.Raycast(transform.position + Vector3.up * groundMinDistance, dir, groundMaxDistance,
                            groundLayer))
                    {
                        isSliding = true;
                    }
        ...
        protected virtual void SlideMovementBehavior()
        {
            ...
            _rigidbody.velocity = Vector3.Lerp(_rigidbody.velocity, dir * slideDownVelocity,
                slideDownSmooth * Time.fixedDeltaTime);
            dir.y = 0f;

            if (_rotateSlopeEnterTime <= 0f)
            {
                Vector3 desiredForward = Vector3.RotateTowards(transform.forward, dir,
                    rotateDownSlopeSmooth * Time.fixedDeltaTime, 0f);
                ...
                if (debugWindow)
                {
                    Debug.DrawRay(transform.position,
                        Vector3.ProjectOnPlane(normal.normalized, groundHit.normal).normalized, Color.blue);
                    Debug.DrawRay(transform.position,
                        Quaternion.AngleAxis(90, groundHit.normal) *
                        Vector3.ProjectOnPlane(normal.normalized, groundHit.normal).normalized, Color.red);
                    Debug.DrawRay(transform.position, transform.TransformDirection(rightMovement.normalized * 2f),
                        Color.green);
                }
            }
            else
        ...
        protected virtual void OnDrawGizmos()
        {
            if (Application.isPlaying && debugWindow)
            {
                ...
                Gizmos.DrawWireSphere(ray1.GetPoint((crouchHeadDetect - (colliderRadius * 0.1f))),
                    colliderRadius * 0.9f);
            }
        }
        ...
        public class vMovementSpeed
        {
            ...
            [Tooltip("Rotation speed of the character")] [FormerlySerializedAs("rotationSpeed")] [SerializeField]
        ...
        public void TakeDamage(int damageAmount, bool b)
        {
