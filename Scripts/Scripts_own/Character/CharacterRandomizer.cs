...
namespace PsychoticLab
{
    ...
    {
        private List<GameObject[]> appearances;
        [Header("Demo Settings")] public bool repeatOnPlay = false;
        public float shuffleSpeed = 0.7f;

        [Header("Material")] public Material mat;

        [Header("Gear Colors")] public Color[] primary =
        {
        ...
        [Header("Metal Colors")] public Color[] metalPrimary =
        {
        ...
        [Header("Skin Colors")] public Color[] whiteSkin = { new Color(1f, 0.8000001f, 0.682353f) };
        ...
        [Header("Hair Colors")] public Color[] whiteHair =
        {
        ...
        {
            new Color(0.2431373f, 0.2039216f, 0.145098f), new Color(0.1764706f, 0.1686275f, 0.1686275f),
        ...
        {
            new Color(0.9764706f, 0.9686275f, 0.9568628f), new Color(0.1764706f, 0.1686275f, 0.1686275f),
            new Color(0.8980393f, 0.7764707f, 0.6196079f)
        };

        public Color elfStubble = new Color(0.8627452f, 0.7294118f, 0.6862745f);

        [Header("Scar Colors")] public Color whiteScar = new Color(0.9294118f, 0.6862745f, 0.5921569f);
        ...
        [Header("Body Art Colors")] public Color[] bodyArt =
        {
        ...
        [HideInInspector] public List<GameObject> enabledObjects = new List<GameObject>();

        // character object lists
        // male list
        [HideInInspector] public CharacterObjectGroups male;

        // female list
        [HideInInspector] public CharacterObjectGroups female;

        // universal list
        [HideInInspector] public CharacterObjectListsAllGender allGender;
        ...
        void OnGUI()
        {
            ...
            */
            ...
            // style.fontSize = 24;
            // GUI.Label(new Rect(10, 10, 150, 50), "마우스 우클릭/A D키를 이용하여 회전", style);
        }
        ...
        public void ChangeAppearance(Gender gender, Human human, int index, int one)
        {
            Dictionary<Human, List<GameObject>> costume = null;
            // enabledObjects[(byte)human].SetActive(false);
            // enabledObjects[(byte)human] = null;
            //
            //
            // CharacterObjectGroups character = null;
            if (gender == Gender.Male)
            {
                costume = costume_male;
            }
            else if (gender == Gender.Female)
            {
                costume = costume_female;
            }

            
                for (int i = 0; i < 13; i++)
                {
                    costume[human][i].SetActive(false);
                }
                // costume[human][index+one].SetActive(false);
                if (human == Human.ArmUp_right)
                {
                    for (int i = 0; i < 13; i++)
                    {
                        costume[human][i].SetActive(false);
                        costume[Human.ArmUp_left][i].SetActive(false);
                    }

                    ActivateItem(costume[Human.ArmUp_right][index], index);
                    ActivateItem(costume[Human.ArmUp_left][index], index);
                }
                else if (human == Human.ArmDown_right)
                {
                    for (int i = 0; i < 13; i++)
                    {
                        costume[human][i].SetActive(false);
                        costume[Human.ArmDown_left][i].SetActive(false);
                    }

                    ActivateItem(costume[Human.ArmDown_right][index], index);
                    ActivateItem(costume[Human.ArmDown_left][index], index);
                }
                else if (human == Human.Hand_right)
                {
                    for (int i = 0; i < 13; i++)
                    {
                        costume[human][i].SetActive(false);
                        costume[Human.Hand_left][i].SetActive(false);
                    }

                    ActivateItem(costume[Human.Hand_right][index], index);
                    ActivateItem(costume[Human.Hand_left][index], index);
                }
                else if (human == Human.Leg_right)
                {
                    for (int i = 0; i < 13; i++)
                    {
                        costume[human][i].SetActive(false);
                        costume[Human.Leg_left][i].SetActive(false);
                    }

                    ActivateItem(costume[Human.Leg_right][index], index);
                    ActivateItem(costume[Human.Leg_left][index], index);
                }
                else
                {
                    ActivateItem(costume[human][index], index);
                }
            ...
            //         {
            //             costume[human][i].SetActive(false);
            //             costume[human + 1][i].SetActive(false);
            //         }
            //
            //         ActivateItem(costume[Human.ArmUp_right][index], index);
            //         ActivateItem(costume[Human.ArmUp_left][index], index);
        ...
        void RandomizeByVariable(CharacterObjectGroups cog, Gender gender, Elements elements, Race race,
            FacialHair facialHair, SkinColor skinColor, HeadCovering headCovering)
        {
            ...
            switch (elements)
            {
                case Elements.Yes:
                    ...
                    if (cog.headAllElements.Count != 0)
                    {
                        ...
                        ActivateItem(cog.headAllElements[a]);
                        GameManager.instance.data.costume[Human.Head] = a;
                        print(GameManager.instance.data.costume[Human.Head]);
                    ...
                    {
                        int a = Random.Range(0, 11);
                        ActivateItem(cog.eyebrow[a]);
                        GameManager.instance.data.costume[Human.Eye] = a;
                    }
                    ...
                    //                     Random.Range(0, allGender.headCoverings_Base_Hair.Count)]);
                    //         break;
                    ...
                    //             ActivateItem(allGender.headCoverings_No_FacialHair[
                    //                 Random.Range(0, allGender.headCoverings_No_FacialHair.Count)]);
                    //         break;
            ...
            {
                int a = Random.Range(0, 11);
                ActivateItem(cog.torso[a]);
                GameManager.instance.data.costume[Human.Body] = a;
            }

            // determine chance for upper arms to be different and activate
            if (cog.arm_Upper_Right.Count != 0)
            {
                // RandomizeLeftRight(cog.arm_Upper_Right, cog.arm_Upper_Left, 11);
                int a = Random.Range(0, 11);
                ActivateItem(cog.arm_Upper_Right[a]);
                ActivateItem(cog.arm_Upper_Left[a]);
                GameManager.instance.data.costume[Human.ArmUp_right] = a;
                GameManager.instance.data.costume[Human.ArmUp_left] = a;
            }

            // determine chance for lower arms to be different and activate
            if (cog.arm_Lower_Right.Count != 0)
            {
                // RandomizeLeftRight(cog.arm_Lower_Right, cog.arm_Lower_Left, 11);
                int a = Random.Range(0, 11);
                ActivateItem(cog.arm_Lower_Right[a]);
                ActivateItem(cog.arm_Lower_Left[a]);
                GameManager.instance.data.costume[Human.ArmDown_right] = a;
                GameManager.instance.data.costume[Human.ArmDown_left] = a;
            }

            // determine chance for hands to be different and activate
            if (cog.hand_Right.Count != 0)
            {
                // RandomizeLeftRight(cog.hand_Right, cog.hand_Left, 11);
                int a = Random.Range(0, 11);
                ActivateItem(cog.hand_Right[a]);
                ActivateItem(cog.hand_Left[a]);
                GameManager.instance.data.costume[Human.Hand_right] = a;
                GameManager.instance.data.costume[Human.Hand_left] = a;
            }
            ...
            {
                int a = Random.Range(0, 11);
                ActivateItem(cog.hips[a]);
                GameManager.instance.data.costume[Human.Hip] = a;
            }

            // determine chance for legs to be different and activate
            if (cog.leg_Right.Count != 0)
            {
                // RandomizeLeftRight(cog.leg_Right, cog.leg_Left, 11);
                int a = Random.Range(0, 11);
                ActivateItem(cog.leg_Right[a]);
                ActivateItem(cog.leg_Left[a]);
                GameManager.instance.data.costume[Human.Leg_right] = a;
                GameManager.instance.data.costume[Human.Leg_left] = a;
            }
        ...
        public void changeWhite()
        {
            ...
            changeSkin(SkinColor.White);
        }
        public void changeBrown()
        {
            changeSkin(SkinColor.Brown);
        }
        public void changeElf()
        {
            changeSkin(SkinColor.Elf);
        }
        public void changeBlack()
        {
            changeSkin(SkinColor.Black);
        }

        public void changeSkin(SkinColor skin)
        {
            switch (skin)
            {
        ...
        void ActivateItem(GameObject go, int index)
        {
            // enable item
            go.SetActive(true);
            // add item to the enabled items list
            enabledObjects[index] = go;
        }
        ...
        private Dictionary<Human, List<GameObject>> costume_all = new Dictionary<Human, List<GameObject>>();
        public Dictionary<Human, List<GameObject>> costume_male = new Dictionary<Human, List<GameObject>>();
        public Dictionary<Human, List<GameObject>> costume_female = new Dictionary<Human, List<GameObject>>();
        ...
        private void BuildLists()
        {
            ...
            costume_female.Add(Human.Head, female.headAllElements);
            costume_female.Add(Human.Eye, female.eyebrow);
            costume_female.Add(Human.Hair, allGender.all_Hair);
            costume_female.Add(Human.Body, female.torso);
            costume_female.Add(Human.ArmUp_right, female.arm_Upper_Right);
            costume_female.Add(Human.ArmUp_left, female.arm_Upper_Left);
            costume_female.Add(Human.ArmDown_right, female.arm_Lower_Right);
            costume_female.Add(Human.ArmDown_left, female.arm_Lower_Left);
            costume_female.Add(Human.Hand_right, female.hand_Right);
            costume_female.Add(Human.Hand_left, female.hand_Left);
            costume_female.Add(Human.Hip, female.hips);
            costume_female.Add(Human.Leg_right, female.leg_Right);
            costume_female.Add(Human.Leg_left, female.leg_Left);

            costume_male.Add(Human.Head, male.headAllElements);
            costume_male.Add(Human.Eye, male.eyebrow);
            costume_male.Add(Human.Hair, allGender.all_Hair);
            costume_male.Add(Human.Body, male.torso);
            costume_male.Add(Human.ArmUp_right, male.arm_Upper_Right);
            costume_male.Add(Human.ArmUp_left, male.arm_Upper_Left);
            costume_male.Add(Human.ArmDown_right, male.arm_Lower_Right);
            costume_male.Add(Human.ArmDown_left, male.arm_Lower_Left);
            costume_male.Add(Human.Hand_right, male.hand_Right);
            costume_male.Add(Human.Hand_left, male.hand_Left);
            costume_male.Add(Human.Hip, male.hips);
            costume_male.Add(Human.Leg_right, male.leg_Right);
            costume_male.Add(Human.Leg_left, male.leg_Left);
        }
